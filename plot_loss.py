"""
plot_loss.py — draws the loss curve (train vs val) from data/train_log.csv.
Generates loss_curve.png to drop into the README.

    python plot_loss.py
"""

import os
import csv
import matplotlib
matplotlib.use("Agg")                                  # windowless backend (just saves a file)
import matplotlib.pyplot as plt

HERE = os.path.dirname(os.path.abspath(__file__))
CSV  = os.path.join(HERE, "data", "train_log.csv")
OUT  = os.path.join(HERE, "loss_curve.png")

# read the CSV that train.py wrote
iters, train, val = [], [], []
with open(CSV) as f:
    for row in csv.DictReader(f):
        iters.append(int(row["iter"]))
        train.append(float(row["train"]))
        val.append(float(row["val"]))

# plot the two curves
plt.figure(figsize=(7, 4.5))
plt.plot(iters, train, label="train", linewidth=2)
plt.plot(iters, val,   label="val",   linewidth=2)
plt.xlabel("iteration")
plt.ylabel("loss (cross-entropy)")
plt.title("modern-nanoGPT — TinyShakespeare")
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(OUT, dpi=120)
print(f"curve saved to {OUT}  (final val: {val[-1]:.4f})")
